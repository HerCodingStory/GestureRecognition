                                    ...
                                    HandModel left_model, HandModel right_model)
    {
        ...
        for (int h = 0; h < num_hands; ++h)
        {
            ...
            if (model != null)
            {
                ...
                if (recorder_.state == RecorderState.Stopped || recorder_.state == RecorderState.Recording)
                    all_hands[leap_hand.Id].gameObject.SetActive(false);
                else
                    all_hands[leap_hand.Id].gameObject.SetActive(true);
            }
        }
