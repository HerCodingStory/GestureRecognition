    ...
    public class CollisionHand : MonoBehaviour
    {
        ...
        void iniCollisionHand()
        {
            for (int i = 0; i < 15; ++i)
            {
                if (bones[i] == null)
                {
                    bones[i] = GameObject.Instantiate(handColliderPrefab).transform;
                }
            ...
            if (palm == null)
            {
                palm = GameObject.Instantiate(handColliderPrefab).transform;
        ...
        void setPalm()
        {
            ...
            {
                palm.position = new Vector3(palmPos.x * transform.lossyScale.x, palmPos.y * transform.lossyScale.y, palmPos.z * transform.lossyScale.z)
                       + transform.position;
